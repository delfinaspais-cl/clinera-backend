...
export class ExportService {
  ...
  async generateTurnosPDF(
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=reporte-turnos-${clinicaName}-${new Date().toISOString().split('T')[0]}.pdf`,
    );
    ...
    doc
      .fontSize(20)
      .text(`Reporte de Turnos - ${clinicaName}`, { align: 'center' });
    doc.moveDown();
    doc
      .fontSize(12)
    ...
    doc
      ...
      .lineTo(startX + colWidth * 7, yPosition)
      .stroke();
    yPosition += 10;
  ...
  async generateTurnosExcel(
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=reporte-turnos-${clinicaName}-${new Date().toISOString().split('T')[0]}.xlsx`,
    );
  ...
  async generatePacientesPDF(
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=reporte-pacientes-${clinicaName}-${new Date().toISOString().split('T')[0]}.pdf`,
    );
    ...
    doc
      .fontSize(20)
      .text(`Reporte de Pacientes - ${clinicaName}`, { align: 'center' });
    doc.moveDown();
    doc
      .fontSize(12)
      .text(`Generado el: ${new Date().toLocaleDateString('es-ES')}`, {
    ...
    doc
      .moveTo(startX, yPosition)
      .lineTo(startX + colWidth * 5, yPosition)
      .stroke();
    yPosition += 10;
  ...
  async generatePacientesExcel(
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=reporte-pacientes-${clinicaName}-${new Date().toISOString().split('T')[0]}.xlsx`,
    );
